...
export default function EditListingPage() {
  ...
  useEffect(() => {
    async function loadListing() {
      ...
      try {
        ...
        setPricePerNight(item.price_per_night);
        setCleaningFee(item.cleaning_fee);
        setServiceFee(item.service_fee);
        setMaxGuests(item.max_guests);
        setBedrooms(item.bedrooms);
        setBeds(item.beds);
        setBaths(item.baths);
        setImages(item.images.map((img) => img.url));
        setSelectedAmenities(item.amenities.map((am) => am.name));
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentUser) return showToast('Please select a Host user profile', 'error');
  ...
  if (isLoading) return <div className="p-8 text-center text-sm font-bold text-gray-500">Loading details...</div>;
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 flex-1 w-full space-y-6">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-200 shadow-sm space-y-6">
          <h1 className="text-2xl font-bold text-gray-900">Edit Listing #{id}</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            ...
            <div>
              ...
              <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-gray-400 p-4 text-sm font-semibold text-gray-700 hover:bg-gray-50">
                <Upload className="w-4 h-4" /> Choose more photos (up to 8)
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              <button type="submit" disabled={isSubmitting} className="bg-airbnb-brand text-white font-bold py-2.5 px-6 rounded-xl shadow">{isSubmitting ? 'Saving...' : 'Save Changes'}</button>
            </div>
          </form>
        </div>
      </main>
